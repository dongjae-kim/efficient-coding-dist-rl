...
def make_illustration(n_neurons=20, logscale=True, save_fig=None,
                      alpha=1, beta=None):
    if beta is None:
        beta = 1 - alpha
    if logscale:
    ...
    mu, sigma = get_pars()
    quant = np.linspace(1 / (2 * n_neurons + 1),
                        2 * n_neurons / (2 * n_neurons + 1), n_neurons)
    ...
    for i in range(n_neurons):
        ...
        plt.plot(x, gain * _sigmoid(scipy.stats.norm.cdf(np.log(x), mu, sigma),
                                    quant[i], sig_sigmoid),
...
def make_illustration_panel(n_neurons=20, logscale=False, save_fig=None,
                            rs=7.71, alpha=0.77, beta=None, gamma=0,
                            slope_scale=5, plot_thresh='none'):
    if beta is None:
        beta = 1 - alpha
    if logscale:
        x_min = 0.1
    ...
    y_cdf /= y_cdf[-1]
    d = (y_pdf ** alpha) / (1 - scipy.stats.norm.cdf(np.log(x), mu, sigma)) ** beta
    ...
    gain_x = (
        (y_pdf ** (1-alpha))
        / ((1 - scipy.stats.norm.cdf(np.log(x), mu, sigma)) ** (1 - beta)))
    ...
    for i in range(n_neurons):
        gain = (
            (np.interp(thresh[i], x, y_pdf) ** (1-alpha))
            / ((1 - scipy.stats.norm.cdf(np.log(thresh[i]), mu, sigma)) ** (1 - beta)))
...
def make_rl_illustration(logscale=False, save_fig=None, alpha=1, beta=None):
    if beta is None:
        beta = 1 - alpha
    if logscale:
        x_min = 0.1
    ...
    p = patches.Polygon([[t0, 0], [t0 + 2, 0], [t0 + 2, 1]],
                        facecolor=[0, 0, 1, 0.5])
    a_cdf.add_patch(p)
    p = patches.Polygon([[t0, 0], [t0 - 2, 0], [t0 - 2, -0.8 / 3]],
                        facecolor=[1, 0, 0, 0.5])
    a_cdf.add_patch(p)
    ...
    for i in range(n_neurons):
        ...
        x = [thresh[i] - (np.cos(angle[0]) * length),
             thresh[i],
             thresh[i] + (np.cos(angle[1]) * length)]
        y = [-np.sin(angle[0]) * length,
             0,
             np.sin(angle[1]) * length]
...
def make_rl_insets():
    ...
    spike = np.array([0, 0.25, 0.5, 1, 0.8, 0.5, 0, -0.1, -0.2, -0.19,
                      -0.175, -0.15, -0.125, -0.1, -0.08, -0.06, -0.04,
                      -0.03, -0.02, -0.01])
    response = noise
    for t in times:
...
def make_solution(n_neurons=10, save_fig=None, alphas=None, use_beta=True,
                  mus=None, sigmas=None, slope_scale=5, x_max=50):
    if alphas is None:
    ...
    if mus is None:
        ...
        assert len(mus) == len(sigmas), 'mu and sigma vectors must be the same length if provided'
    fig = plt.figure(figsize=[len(mus) * 2.5 + 1.5, (len(alphas) + 1) * 1.25 + 0.5])
    x_min = np.finfo(float).eps
    x = np.linspace(x_min, x_max, 1000)
    for j, [mu, sigma] in enumerate(zip(mus, sigmas)):
        ...
        for i, alpha in enumerate(alphas):
            ...
            plot_optimal(ax, x, y_pdf, y_cdf, alpha, n_neurons, mu, sigma,
                         use_beta=use_beta, slope_scale=slope_scale)

    if save_fig is not None:
        plt.savefig(save_fig, format='pdf', bbox_inches='tight')


def plot_optimal(ax, x, y_pdf, y_cdf, alpha, n_neurons, mu, sigma,
                 use_beta=True, slope_scale=None, color='k'):
...
def make_efficient(n_neurons=10, save_fig=None, alpha=1, beta=None,
                   slope_scale=None, use_beta=False):
    if beta is None:
        beta = 1 - alpha
    if slope_scale is None:
        slope_scale = n_neurons + 1
    ...
    plt.bar(np.arange(5), expected_info-expected_info[0],
            color=colors)
    ...
    a_bar.set_xticklabels(['one neuron', 'equal spacing', 'no gain', 'optimal',
                           'optimal_alpha'])
