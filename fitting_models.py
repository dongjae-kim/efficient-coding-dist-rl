...
class value_dist_rl(value_efficient_coding_moment):
    def __init__(
        self, ec, alphas, quantiles, thresholds, r_star, N_neurons=40, dir_save="drl"
    ):
        ...
        for i in range(len(self.juice_prob)):
            temp_ = norm.pdf(self.x_log, np.log(self.juice_magnitudes[i]), 0.2)
        ...
        for i in range(len(self.juice_prob)):
            temp_ = norm.pdf(self.x_log_inf, np.log(self.juice_magnitudes[i]), 0.2)
            sum_pdf += temp_ / np.max(temp_) * self.juice_prob[i]
        ...
        for i in range(N_neurons):
            ...
            # each_neuron_array[
            #    np.where(each_neuron >= np.max(self.ec.neurons_[i]))
            # ] = np.max(self.ec.neurons_[i])
    ...
    def init_figures(self, iternum):
        ...
        # plt.show()
        plt.savefig(
            self.dir_save + "Mixed Response Function_{0:04d}.png".format(iternum)
        )
        ...
        for i in range(self.N):
            ...
            for j in range(len(self.x)):
                ...
                if self.x[j] < self.thresholds[i]:
                    # slope * (x - threshold) + r_star
                    each_neuron.append(
                        self.alphas[i][0] * (self.x[j] - self.thresholds[i])
                        + self.r_star
                    )
                else:
                    each_neuron.append(
                        self.alphas[i][1] * (self.x[j] - self.thresholds[i])
                        + self.r_star
                    )
        ...
        for i in range(self.N):
            ...
            for j in range(len(self.x)):
                ...
                if self.x[j] < self.thresholds_mixed[i]:
                    # slope * (x - threshold) + r_star
                    each_neuron.append(
                        self.alphas[i][0] * (self.x[j] - self.thresholds_mixed[i])
                        + self.r_star
                    )
                else:
                    each_neuron.append(
                        self.alphas[i][1] * (self.x[j] - self.thresholds_mixed[i])
                        + self.r_star
                    )
            # each_neuron_array = np.array(each_neuron)
            # each_neuron_array[
    ...
    def plot_thresholds(self, iternum):
        ...
        sns.kdeplot(self.thresholds, bw=0.75, color="k", lw=3.0, shade=True)
        ...
        plt.figure(999)
        sns.set_style("whitegrid")
        sns.kdeplot(self.thresholds_mixed, bw=0.75, color="k", lw=3.0, shade=True)
...
def neuron_neuron_offset(ec, neuron, start_offset, maxval=118.0502):
    ...
    while flag < maxval:
        ...
        for i in range(len(neuron)):
            neuneurons_.append((neuron[i, :] + OFFSET).tolist())
...
class value_efficient_coding_fitting_sd(value_efficient_coding_moment):
    def __init__(self, N_neurons=18, R_t=247.0690, XX2=1.0):
        ...
        # since we posit a distribution ranged in [0,20] (mostly)
        # we hypothesized that integral from -inf to +inf is same
        # as the integral from 0 to 20 in this toy example.
        # From now on, we just calculated cumulative distribution using
        # self.x, which ranged from 0 to 20.
        # a prototype sigmoidal response curve
        ...
        norm_d_pseudo = self.p_prior_pseudo**XX2 / (1 - self.cum_P_pseudo) ** (
            1 - XX2
        )
        NRMLZR_pseudo = np.sum(norm_d_pseudo * self._x_gap)
        norm_d_pseudo = norm_d_pseudo / NRMLZR_pseudo
        ...
        #     # self.d_x.append(self.d(i)) # based on the assumption that our domain ranged [0,20]
        #  is approximately same as [-inf, inf]
        #
        ...
        # preferred response of each neuron.
        # It is x=0 in the prototype sigmoid function (where y=0.5)
        self.sn = []
        self.sn_pseudo = []

        self.D_pseudo = []
        # multiply _x_gap to approximate continuous integral.
        # self.D = np.cumsum(self.d_x * self._x_gap)
        ...
        ind_sets_pseudo = []
        for i in range(self.N):
            ind_set = np.argmin(np.abs(np.round(cum_norm_D + 0.5) - (i + 1)))
            ...
            ind_set = np.argmin(np.abs((cum_norm_D_pseudo + 0.5) - (i + 1)))
        ...
        for i in range(self.N):
            ...
            for j in range(len(self.x_inf)):
                # hn_inner_integral.append(
                #    self.d_x_pseudo[j]
                #    * h_prime(self.D_pseudo[j] - (i + 1))
                #    * self._x_gap
                # )
                hn_inner_integral.append(
                    norm_d_pseudo[j]
                    * h_prime(cum_norm_D_pseudo[j] - (i + 1))
                    * self._x_gap
                )
            h_n = g_sn * np.cumsum(hn_inner_integral)
...
class fitting_model_model:
    ...
    def neuron_R_fit_fixed_rstar(self, XX):
        ...
        (
            ...
            xs,
            ys,
        ) = fit_.plot_approximate_kinky_fromsamples_fitting_only(
            self.samples_idx,
            fit_.neurons_,
            name=self.dir_save_figures,
            r_star_param=0.8,
        )
...
def test():
    ...
    for ii in range(1000):
        try:
            ...
            """just skip if file does not exist"""
    ...
    ax.set_xticks([0.1, 0.3, 1.2, 2.5, 5, 10, 20])
    # ax.set_xticks([.1, .3, 1.2, 2.5, 5, 10, 20, ], ['.1', '.3', '1.2', '2.5', '5', '10', '20'])
    ax.set_xlim([0, 12])
    ax.set_ylim([0, 1])
    # ax.set_xlim([0, 15])
    ...
    plt.savefig("Fitting results asym2_default.pdf")
    plt.savefig("Fitting results asym2_default")

    RPSS = ec_moment.get_quantiles_RPs(np.linspace(0, 1, 1000))
    # ax.plot(np.linspace(0,1,1000), RPSS, '--', color=[.7,.7,.7])
    ax.plot(RPSS, np.linspace(0, 1, 1000), "--", color=[0.7, 0.7, 0.7])

    plt.savefig("Fitting results asym2 dotted_default.pdf")
    ...
    for j in range(data["ps_total"][indices].shape[1]):
        ...
        for i in range(len(ec.neurons_)):
            neurons_all_.append(sigmoid_func(ec.x, *data["ps_total"][indices][i, j, :]))
        neurons_all.append(neurons_all_)
    RSUM_all = [
        np.sum(np.array(neurons_all[i]) * ec.p_prior * ec._x_gap)
    ...
    ax2.plot(ec.x_inf, ec.g_x_pseudo, "-", linewidth=4, c="#9ebcda")
    ...
    fig2.savefig("RP2_data_90__default.png")
    fig2.savefig("RP2_data_90__default.pdf")
    ...
    plt.savefig("Fitting results asym22_default.pdf")
    plt.savefig("Fitting results asym22_default")

    RPSS = ec_moment.get_quantiles_RPs(np.linspace(0, 1, 1000))
    # ax.plot(np.linspace(0,1,1000), RPSS, '--', color=[.7,.7,.7])
    ax.plot(RPSS, np.linspace(0, 1, 1000), "--", color=[0.7, 0.7, 0.7])

    plt.savefig("Fitting results asym2 dotted2_default.pdf")
    ...
    for count, R in enumerate(np.sort(RSUM_all)):
        ...
        ax4.plot(ec.x_inf, ec.g_x_pseudo, "-", linewidth=1, c="#9ebcda", alpha=1)
    ...
    ax4.set_xticks([0.1, 0.3, 1.2, 2.5, 5, 10, 20])

    ax4.scatter(
        zero_crossings_estimated, param_set[1][:, 1][idx_sorted_], s=10, c=[0, 0, 0]
    )
    ax4.plot(hires_x, func(hires_x, *best_fit_ab), "black")
    ax4.fill_between(hires_x, bound_lower, bound_upper, color="black", alpha=0.15)
    ...
    ax4.set_xlim(0, 12)
    ax4.set_ylim(0, 40)
    fig4.savefig("RP4_data_90__default.png")
    fig4.savefig("RP4_data_90__default.pdf")
